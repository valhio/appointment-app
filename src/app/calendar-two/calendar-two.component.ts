...
export default class CalendarTwoComponent implements OnInit, OnDestroy {
  ...
  generateDataForCalendar(): void {    
    ...
    this.dateRef.collection('data').get().subscribe(querySnapshot => {
      ...
      this.bookings = querySnapshot.docs.map(doc => doc.id);
  ...
  openCreateBookingDialog(bookingTime: any) {
    const dialogRef = this.dialog.open(AddEventDialogComponent, {
      data: { date: this.selectedDateSubject.value, bookingTime: bookingTime, numberOfBookedBookings: this.bookings.length }
      , maxHeight: '90vh'
    })
  }

  isBookingHourBooked(bookingHour: any): boolean {
    return this.bookings.find(booking => booking == bookingHour) ? true : false;
  }
